 ...
 * 当前页面获取区域代码
 */
export class CurrentPageCountyCodeGetter implements ICountyInfoGetter {
    ...
    getCountyCode(): Promise<string> {
        console.info(format('通过 当前页面 获取区域代码...'))
            ...
            document.querySelectorAll('script').forEach(scriptEl => {
                if (scriptEl.innerText.includes('$J( InitMiniprofileHovers );')) {
                    ...
                    if (match) {
                        ...
                        console.info(format('通过 当前页面 获取区域代码成功：' + countyCode))
                        resolve(countyCode)
                    }
                }
            })
            throw Error(format('通过 当前页面 获取区域代码失败。'))
        })
    }
